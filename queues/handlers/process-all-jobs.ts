...
export const processAllJobs = async () => {
  ...
  for (const [queueType, config] of Object.entries(services)) {
    ...
    let processingCount = (
      await JobRepository.listJobsByQueueTypeAndStatus(queueType, "processing")
    ).length
    ...
    await Promise.all(
      toLaunch.map((job) => {
        ...
        return QueueOrchestrator.processJob(job, config.startWorker)
      })
    )
      ...
      for (const job of processingJobs) {
        if (job.externalId) {
          ...
          await QueueOrchestrator.processJob(
            job,
            async () => config.pollWorker!(job.externalId!),
            false
          )
        }
      }
