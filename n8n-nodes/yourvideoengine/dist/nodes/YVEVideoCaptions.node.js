...
class YVEVideoCaptions {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const color = this.getNodeParameter("color", i, "#fff");
            const fontSize = this.getNodeParameter("fontSize", i, 90);
            const fontFamily = this.getNodeParameter("fontFamily", i, "Montserrat");
            const fontWeight = this.getNodeParameter("fontWeight", i, "black");
            const animationType = this.getNodeParameter("animationType", i, "bump");
            const top = this.getNodeParameter("top", i, 75);
            const uppercase = this.getNodeParameter("uppercase", i, false);
            // Extract advanced options (with fallback)
            const getAdv = (key, fallback) => style && style[key] !== undefined ? style[key] : fallback;
            let wordsArr = [];
            if (resource === "video") {
                ...
                if (operation === "captionWordByWord") {
                    try {
                        ...
                        const inputProps = {
                            ...
                            combineTokensWithinMilliseconds: getAdv("combineTokensWithinMilliseconds", 1400),
                            transitionDuration: getAdv("transitionDuration", "0.12s"),
                            transitionEasing: getAdv("transitionEasing", "cubic-bezier(0.4,0,0.2,1)"),
