...
export async function startFlux(params: FluxParams): Promise<{ externalId: string }> {
  ...
  const inputData: Record<string, any> = {
    ...
    ...(params.control_net_settings ? { control_net_settings: params.control_net_settings } : {}),
  ...
  const data = (await res.json()) as { data: { task_id: string }; error?: string };
...
export async function checkFluxStatus(externalId: string): Promise<{
  ...
  if (status === 'completed') {
    return {
      status: 'ready',
      outputData: data,
      returnData: data.data?.output?.image_url ? { url: data.data.output.image_url } : undefined,
    };
